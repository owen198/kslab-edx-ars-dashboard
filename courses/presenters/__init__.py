...
class BasePresenter_snail(object):
    
    def __init__(self, timeout=settings.ANALYTICS_API_DEFAULT_TIMEOUT):
        self.client = Client(base_url=settings.DATA_API_URL_SNAIL, auth_token=settings.DATA_API_AUTH_TOKEN_SNAIL, timeout=timeout)
    ...
    def risk_response(self):
        return self.client.get('sparkml/atriskstudents/')

class BasePresenter_snail2(object):
